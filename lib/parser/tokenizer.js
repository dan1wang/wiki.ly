 ...
 */
function PegTokenizer(tokenizer, env, options) {
  events.EventEmitter.call(this);
  this.tokenizer = tokenizer ?? null;
  this.env = env;
  this.options = options || { tokenizer };
  this.offsets = {};
}
...
PegTokenizer.prototype.getTokenizerSource = async function(redirects, protocols) {
  const srcFiles = [
    ['wiki.pegjs', content => content.replace('"%REDIRECTS%"', redirects)],
    ['wikilink.pegjs', content => content.replace('"%PROTOCOLS%"', protocols)],
    ['wikitemplate.pegjs'],
    ['wikitag.pegjs'],
    ['wikilist.pegjs'],
    ['wikitable.pegjs'],
  ...
  const srcContent = []
  for (const fileInfo of srcFiles) {
    const [file, process] = fileInfo
    const content = await fs.readFile(path.join(__dirname, file), 'utf8')
    srcContent.push(process ? process(content) : content)
  }
  return srcContent.join('')
}

PegTokenizer.prototype.compileTokenizer = async function() {
  ...
  const src = await this.getTokenizerSource(redirects, protocols)
...
PegTokenizer.prototype.getCompiledTokenizer = async function() {
  const tokenizerSource = await this.compileTokenizer();
